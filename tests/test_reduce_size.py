...
class TestReduceSize(TestCase):
    def test_reduce_size_long(self):
        # GIVEN
        message = ReduceSizeLong.from_parms(time_offset=447_000,
                                            order_id='ORID0001',
                                            canceled_quantity=20_000)
        ...
        assert_that(msg_bytes, is_(compare_bytes(bytearray([
            ...
            0x20, 0x4E, 0, 0  # Canceled Quantity
        ]))))

    def test_reduce_size_short(self):
        # GIVEN
        message = ReduceSizeShort.from_parms(time_offset=447_000,
                                             order_id='ORID0001',
                                             canceled_quantity=20_000)

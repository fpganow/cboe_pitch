...
class TestOrderBook(TestCase):
    ...
    def test_add_order(self):
        ...
        # THEN
        assert_that(ob.has_order_id(ticker=ticker,
                                    side=side,
                                    order_id="ORID0001"), equal_to(True))
    ...
    def test_add_multiple_buy_orders(self):
        ...
        # THEN
        assert_that(ob.has_order_id(ticker=ticker, side=side,
                            order_id="ORID0001"), equal_to(True))
        assert_that(ob.has_order_id(ticker=ticker, side=side,
                                    order_id="ORID0002"), equal_to(True))
        assert_that(ob.has_order_id(ticker=ticker, side=side,
                                    order_id="ORID0003"), equal_to(True))
    ...
    def test_delete_order(self):
        ...
        assert_that(ob.has_order_id(ticker=ticker, side=side,
                                    order_id="ORID0001"), equal_to(True))
        assert_that(ob.has_order_id(ticker=ticker, side=side,
                                    order_id="ORID0002"), equal_to(False))
        assert_that(ob.has_order_id(ticker=ticker, side=side,
                                    order_id="ORID0003"), equal_to(True))
